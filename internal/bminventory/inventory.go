...
func (b *bareMetalInventory) updateHostFencing(ctx context.Context, host *common.Host, fencingCredentialsParams *models.FencingCredentialsParams, cluster *common.Cluster, db *gorm.DB) error {
	...
	//TODO: maybe remove this for now in order to include it in the featuresupport PR
	if getPlatformType(cluster.Platform) != string(models.PlatformTypeBaremetal) && getPlatformType(cluster.Platform) != string(models.PlatformTypeNone) {
		err = errors.Errorf("Cannot set fencing credentials to host %s in infra-env %s, it must be bound to a cluster with either baremetal platform or none platform", host.ID, host.InfraEnvID)
		log.Error(err)
		return common.NewApiError(http.StatusBadRequest, err)
	}
