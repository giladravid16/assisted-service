...
type OsImageResolver interface {
	...
	// GetOsImageForVersion resolves an OS image from an explicit version string that may be
	// either an RHCOS version or an OpenShift version. When the version is an OpenShift
	// version or empty, the corresponding (or latest) release image is used to find the
	// default RHCOS OS image.
